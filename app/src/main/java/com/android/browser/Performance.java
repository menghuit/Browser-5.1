...
import imports.android.net.WebAddress;
...
//        if (Process.readProcFile("/proc/stat", SYSTEM_CPU_FORMAT, null, sysCpu, null)) {
    ...
    private static boolean readProcFile(long[] sysCpu) {
        Class<?> threadClazz = null;
        try {
            threadClazz = Class.forName("android.os.Process");
            Method method = threadClazz.getMethod("readProcFile", String.class, int[].class, String[].class, long[].class, float[].class);
            return (boolean) method.invoke(null, "/proc/stat", SYSTEM_CPU_FORMAT, null, sysCpu, null);
