...
//        }
    ...
    protected void capture() {
        ...
        if (WebViewCompat.getContentWidth(mMainView) <= 0 || mMainView.getContentHeight() <= 0) {
            return;
        }
        Canvas c = new Canvas(mCapture);
        final int left = mMainView.getScrollX();
        final int top = mMainView.getScrollY() + WebViewCompat.getVisibleTitleHeight(mMainView);
        int state = c.save();
