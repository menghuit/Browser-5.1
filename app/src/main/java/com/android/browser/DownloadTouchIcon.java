...
class DownloadTouchIcon extends AsyncTask<String, Void, Void> {
    ...
    public Void doInBackground(String... values) {
        ...
        if (inDatabase || mMessage != null) {
            ...
            try {
                ...
                HttpHost httpHost = /*Proxy.getPreferredHttpHost(mContext, url)*/getHost(mContext);
                if (httpHost != null) {
    ...
    private HttpHost getHost(Context ctx) {
        String proxyAddress;
        int proxyPort;
        ...
        if (proxyAddress != null && proxyPort != -1) {
            return new HttpHost(proxyAddress, proxyPort, "http");
        }
        return null;
    }
