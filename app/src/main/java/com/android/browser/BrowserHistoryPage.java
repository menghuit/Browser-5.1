...
//                Browser.sendString(activity, url, activity.getText(R.string.choosertitle_sharevia).toString());
                sendString(activity, url, activity.getText(R.string.choosertitle_sharevia).toString());
                return true;
    ...
    private void sendString(Context ctx, String url, String chooserTitle) {
        Class<?> clazz = null;
        try {
            clazz = Class.forName("android.provider.Browser");
            final Method sendString = clazz.getDeclaredMethod("sendString", Context.class, String.class, String.class);
            sendString.setAccessible(true);
            sendString.invoke(null, ctx, url, chooserTitle);
        } catch (Exception e) {
            e.printStackTrace();
            Browser.sendString(ctx, url);
        }
    }
