...
import imports.android.provider.BrowserContract.Bookmarks;
...
import com.android.browser.extras.ContextCompat;
...
public class BookmarkThumbnailWidgetService extends RemoteViewsService {
    ...
    static void deleteWidgetState(Context context, int widgetId) {
        File file = /*context.getSharedPrefsFile*/
                ContextCompat.getSharedPrefsFile(context,
                String.format("widgetState-%d", widgetId));
        if (file.exists()) {
            if (!file.delete()) {
                file.deleteOnExit();
    ...
    static void removeOrphanedStates(Context context, int[] widgetIds) {
        File prefsDirectory = /*context.getSharedPrefsFile("null")*/ContextCompat.getSharedPrefsFile(context, "null").getParentFile();
    ...
    static class BookmarkFactory implements RemoteViewsService.RemoteViewsFactory {
        ...
        public RemoteViews getViewAt(int position) {
            ...
            if (isFolder) {
                ...
                setDrawableParameters(views, R.id.thumb, true, 0, -1, null, -1);
            } else {
                ...
                setDrawableParameters(views, R.id.thumb, true, 255, -1, null, -1);
        ...
        private void setDrawableParameters(RemoteViews views, int viewId, boolean targetBackground, int alpha,
                                           int colorFilter, PorterDuff.Mode mode, int level) {
            try {
                final Method setDrawableParameters = views.getClass().getDeclaredMethod("setDrawableParameters",
                        int.class, boolean.class, int.class, int.class, PorterDuff.Mode.class, int.class);
                setDrawableParameters.setAccessible(true);
                setDrawableParameters.invoke(views, viewId, targetBackground, alpha, colorFilter, mode, level);
